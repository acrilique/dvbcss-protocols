 ...
 */
export const BinarySerialiser: ProtocolSerialiser & { toHex(buffer: ArrayBuffer): string } = {
    /**
     ...
     */
    pack: function(wcMsg: WallClockMessage): ArrayBuffer {
        if (wcMsg.version != 0) { throw "Invalid message version"; }

        // create the UDP message to send
        const udpMsg = new Uint8Array(32);
        const d = new DataView(udpMsg.buffer);
        ...
        const t2 = WallClockMessage.nanosToSecsAndNanos(wcMsg.receive_timevalue);
        d.setUint32(16, t2[0]);
        d.setUint32(20, t2[1]);

        const t3 = WallClockMessage.nanosToSecsAndNanos(wcMsg.transmit_timevalue);
        d.setUint32(24, t3[0]);
        d.setUint32(28, t3[1]);
     ...
     * @param msg - The received serialsed message.
     * @returns Object representing the Wall Clock protocol message.
     */
    unpack: function(msg: ArrayBuffer): WallClockMessage {
     ...
     */
    toHex: function(buffer: ArrayBuffer): string {
...
export default BinarySerialiser;
