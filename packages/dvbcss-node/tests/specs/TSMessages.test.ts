...
describe('PresentationTimestamps', () => {
	...
	it('can be serialised and deserialised without loosing information', () => {
		const ts_before = new PresentationTimestamps(
			new PresentationTimestamp(1, 2),
			new PresentationTimestamp(3, 4),
			new PresentationTimestamp(5, 6)
		);
		const ts_after = PresentationTimestamps.deserialise(ts_before.serialise());
...
describe('ControlTimestamp', () => {
	it('exists', () => {
		expect(ControlTimestamp).toBeDefined();
	});

	it('check if ControlTimestamp can be created, serialised and deserialed.', () => {
		const cts_before = new ControlTimestamp(1, 2, 0.5);
		const cts_after = ControlTimestamp.deserialise(cts_before.serialise());
...
describe('TSSetupMessage', () => {
	it('exists', () => {
		expect(TSSetupMessage).toBeDefined();
	});

	it('check if TSSetupMessage can be created, serialised and deserialised.', () => {
		const contentIdStem = 'http://mydomain.org/video.mp4';
		const timelineSelector = 'urn:dvb:css:timeline:ct';

		const tssetup_before = new TSSetupMessage(contentIdStem, timelineSelector);
		const tssetup_after = TSSetupMessage.deserialise(tssetup_before.serialise());

		// do a type check
		expect(tssetup_before).toBeInstanceOf(TSSetupMessage);
		expect(tssetup_after).toBeInstanceOf(TSSetupMessage);
