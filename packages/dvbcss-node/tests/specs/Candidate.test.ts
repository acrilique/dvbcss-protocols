...
describe("Candidate", () => {
    ...
    it("is created from a response message and encapsulates data in units of nanoseconds", () => {
        const msg = new WallClockMessage(0, WallClockMessageTypes.response, 0.1, 256.7, 2000, 145245, 3000.35, 4000.12345);
        
        const candidate = new Candidate(msg, 4000000145245);
    ...
    it("can be converted to a correlation", () => {
        const mockClock = {
            getRootMaxFreqError: () => 20,
            fromNanos: (v: number) => v / 1000000.0,  // uses milliseconds
            parent: {
                fromNanos: (v: number) => v / 1000.0  /// parent uses microseconds
            }
        } as unknown as CorrelatedClock;
        
        const msg = new WallClockMessage(0, WallClockMessageTypes.response, 0.1, 50.0, 2000, 0, 5050, 5060);
        
        const candidate = new Candidate(msg, 2070000000000);
        const correlation = candidate.toCorrelation(mockClock);
