...
describe("Correlation", function() {
	...
	it("is immutable. parentTime, childTime, initialError and errorGrowthRate properties are read-only", function() {
		const c = new Correlation(1,2,3,4);
		
		// The following lines would cause a compile-time error, so we can't even test for a runtime error.
		// c.parentTime=10;
		// c.childTime=10;
		// c.initialError=10;
		// c.errorGrowthRate=10;

		expect(c.parentTime).toBe(1);
		expect(c.childTime).toBe(2);
		expect(c.initialError).toBe(3);
		expect(c.errorGrowthRate).toBe(4);
	});
